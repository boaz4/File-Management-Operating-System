...
class FileManager {
    ...
    static int offcreated = 10, offmod = 22, offsize = 34, offpermi = 37, offfirstcluster = 38, offEnd = 41,
            diskDataStartingPoint = 1048020;
        ...
        while (true) {
            ...
            if (s[0].equals("createf")) {
                if (s.length < 2) {
                    ...
                    System.out.println(
                            "File with name \"" + s[1] + "\" already exists. Try again with different name...");
                    continue;
                }
                char p = permi();
                ...
                if (s.length < 2) {
                    ...
                    if (s.length < 3) {
                        ...
                        if (per[1] == 1) {
                            ...
                            int totalFreeBytes = clustersize * pqfat.size() + clustersize
                                    - basetodec(getDataFromFileEntry(index.get(s[2]), offEnd, 3));// Check whether file
                            // exists?
                            if (totalFreeBytes >= sb.toString().length()) {
                                ...
                                System.out.println(
                                        "Error! Not Enough free space left in the disk! Delete some data and try again . . .");
                        } else {
                    ...
                    if (s.length < 3) {
                        ...
                        if (per[1] == 1) {
                            ...
                            if (sb.toString().length() == 0)
                                continue;
                            String sbb = sb.toString().substring(0, sb.length() - 1);
                            int totalFreeBytes = clustersize
                                    * (pqfat.size() + basetodec(getDataFromFileEntry(index.get(s[2]), offsize, 3)));
            ...
            } else if (s[0].equals("copyf")) {
                if (s.length < 2) {
                    System.out.println("Not enough argument. Try again ...");
                    continue;
                } else if (index.containsKey(s[2])) {
                    System.out.println(
                            "File with name \"" + s[2] + "\" already exists. Try again with different name...");
                    continue;
                } else if (s[1].length() <= 10) {
                    if (index.containsKey(s[1])) {
                        StringBuilder con = new StringBuilder();
                        copyFile(s[1], s[2], con);
                    } else {
                        System.out.println("Sorry! File with this name doesn't exist! Try Again . . .");
                    }
                }
    ...
    static int[] getPer(char c) {
        ...
        for (int i = 0; permissions.length() < 3; i++)
            permissions += 0;
        return new int[] { (int) permissions.charAt(2) - '0', (int) permissions.charAt(1) - '0',
                (int) permissions.charAt(0) - '0' };
    }
    ...
    static void copyFile(String sourceFile, String destinationFile, StringBuilder con) throws Exception {
        readFile(sourceFile, con);
        char p = permi();
        int is = createFile(destinationFile, p);
        if (is == 1)
            System.out.println("New File created successfully...");
        else if (is == -1)
            System.out.println("Error...Sytem was unable to create new File. Try Again...  ");

        char pp = getDataFromFileEntry(index.get(destinationFile), offpermi, 1).charAt(0);
        int[] per = getPer(pp);
        if (per[1] == 1) {
            int address = getAddressFromFileEntry(index.get(destinationFile));
            file.seek(address + offmod);
            file.write(getCurrDateAndTime().getBytes());
            String sbb = con.toString().substring(0, con.length());
            int totalFreeBytes = clustersize * pqfat.size() + clustersize
                    - basetodec(getDataFromFileEntry(index.get(destinationFile), offEnd, 3));
            // Check whether file
            // exists?
            if (totalFreeBytes >= con.toString().length()) {
                appendFile(destinationFile, sbb.getBytes());
                // System.out.println();
            } else {
                System.out
                        .println("Error! Not Enough free space left in the disk! Delete some data and try again . . .");
            }
        } else {
            System.out.println("Sorry! This file has no write permissions.");
        }
    ...
    static int createFile(String name, char permissions) throws Exception {
        ...
        if (name.length() > 10 || (int) permissions > 55 || pqfE.size() == 0 || pqfat.size() == 0)
            return -1;
